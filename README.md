...
目前 Android 版本：`0.120.0`（`versionCode 120`）。
...
- 支援失敗或取消任務重新排入下載。
- 支援清除已完成、失敗、取消的任務記錄。
- 支援將完成檔匯出到公開下載目錄 `Downloads/AI Test Downloader`。
...
- DASH 已能偵測獨立音訊軌並在狀態中提示目前音訊 mux 尚未完整移植。
...
- 完整 FFmpeg/ffprobe 等價能力：多軌 DASH 音訊/影片 mux、轉碼 fallback、duration/完整性驗證仍需繼續補齊。
- 更深層的單站解析器：MovieFFM、Gimy、Xiaoya、JAV/adult 站群仍需持續移植 Windows 版的深層站點邏輯。
