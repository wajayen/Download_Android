...
- v0.218.0: 修正按下搜尋/下載後搜尋列表遲遲不出現的問題；搜尋輸入會先去除 `.mp4` 等檔名尾巴，站內搜尋一取得可用結果就立即顯示列表，不再等待後續慢速站台或 DuckDuckGo 逾時。已用「追夢人」與「追夢人.mp4」驗證可列出 MovieFFM 標題、來源與縮圖結果。
- v0.219.0: 修正 MovieFFM 搜尋結果選取後下載全部失敗的流程；Android 版現在支援新版 `movieffm.me` 的 `/ffm/{id}/{ep_slug}` AJAX 播放 API，會用瀏覽器式 XHR headers 讀取 `video_plays[].play_data`，取得實際 HLS 候選後再交給既有下載進度與系統通知流程。
- v0.220.0: 進一步修正 MovieFFM 搜尋列表品質；搜尋結果補齊標題與縮圖時會檢查頁面是否真的有播放來源 tab、播放器 metadata 或直接媒體線索，沒有可下載來源的頁面不再出現在候選清單，避免使用者選到必然失敗的結果。
- v0.221.0: MovieFFM 直接頁面解析失敗時會用頁面標題重新搜尋同片名候選，並嘗試同站其他可下載結果；這補上 Windows 版同片名備援的 Android 等價流程，避免舊搜尋結果或外部分享頁面沒有播放來源時立刻失敗。
- v0.222.0: 搜尋結果下載若選中的候選來源失敗，服務層會自動切換到下一個尚未嘗試的持久化候選來源；同批候選全部失敗後才把任務標記為失敗，讓 Android 版更接近 Windows 版的多來源備援下載流程。
- v0.223.0: 修正 MovieFFM 搜尋結果誤判可下載的問題；單純只有 WordPress postid、沒有實際 ep_slug、video_plays、play_data 或媒體 URL 的頁面不再進入搜尋結果，且直接站內搜尋只有少量候選時會繼續補 DuckDuckGo 備援來源，避免單一死頁造成下載流程全部失敗。
- v0.224.0: 搜尋列表新增高風險站台的輕量可解析性預檢；MovieFFM、Gimy、XiaoyaKankan、MacCMS 類站台與動畫站結果若無法抽到播放頁、iframe、API 或媒體候選，會在顯示前排除，減少選到頁面存在但下載必敗的結果。
- v0.225.0: 頁面解析流程改為同層候選逐一嘗試；當第一個播放頁、iframe 或 API 候選解析失敗時，會自動改試同頁其他候選，降低單一壞播放線造成搜尋後下載失敗的機率。
- v0.226.0: 解析階段會把每個候選來源的 Referer 一起保存；從來源候選面板或自動備援改試 HLS、API、iframe、播放器候選時，能維持正確來源頁脈絡，改善受 Referer 保護站台的重試成功率。
- v0.227.0: 頁面解析若先嘗試播放器頁、iframe 或 API 候選失敗，會退回同批候選中的直接 HLS、DASH、MP4、WebM、M4V 媒體網址，不再因第一順位播放器頁壞掉而忽略後面的直接媒體來源。
- v0.228.0: 替代媒體來源下載重試時會同步保存剩餘候選的 Referer，且候選清單更新為空標籤或空 Referer 時會清除舊資料，避免來源候選面板與後續重試使用錯位的舊 Referer。
- v0.229.0: 手動重試失敗任務、手動改試下一個來源、或從來源候選面板指定來源時，會清除上一輪自動備援留下的 failedCandidateUrls，避免新一輪下載被舊失敗紀錄誤判為已嘗試。
- v0.230.0: 重試失敗任務、切換來源或自動備援改試下一個候選時，會重置 50MB 後播放狀態，避免上一輪播放已啟動或播放失敗的紀錄讓新一輪下載無法再次觸發播放。
- v0.231.0: 候選來源保存上限從 12 筆提高到 24 筆，且候選標籤與 Referer 會保留空值位置，不再因中間空白被移除而和 URL 清單錯位。
- v0.232.0: 候選 URL、標籤與 Referer 改為同一輪同步寫入；只有 URL 有效的候選才會保存對應標籤與 Referer，並以實際保存筆數作為候選數，避免空 URL 造成三個陣列再次錯位。
- v0.233.0: 搜尋結果剛建立候選清單時就同步保存每筆搜尋結果的 Referer，來源候選面板與解析前重試也能使用正確搜尋來源頁脈絡。
- v0.234.0: 手動下一來源、自動備援下一來源、或指定來源候選時，若任務檔名仍是 `download.bin`、`video.mp4` 等泛用名稱，會依新候選 URL 重新推導較合適的檔名。
- v0.235.0: 重試失敗或取消任務時會清除上一輪輸出路徑，讓新一輪任務狀態只保留目前下載的進度、錯誤與輸出結果，避免舊輸出殘留影響 UI 或後續流程。
- v0.236.0: 重試失敗或取消任務時，若檔名仍是 `download.bin`、`video.mp4` 等泛用名稱，會依目前 resolvedUrl 或 url 重新推導檔名，讓一般重試也和來源切換重試一樣維持較準確的隊列、通知與輸出名稱。
- v0.237.0: HTTP 續傳遇到伺服器回應 416 Range Not Satisfiable 時，會判定舊部分檔已失效，刪除 `.part` 與 `.httpstate` 後自動重新從頭下載，避免可恢復的續傳失效被直接標成下載失敗。
