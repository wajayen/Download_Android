...
- HTTP, HLS, DASH, and HLS-remuxed MP4 final outputs now choose a unique filename when the target already exists instead of replacing an existing completed download.
...
- Android input accepts shared `content://` and `file://` file URIs from Telegram, LINE, and other apps through `ACTION_SEND` and `ACTION_SEND_MULTIPLE`, importing them through the same queue and output-export path as network downloads.
- Shared local-file imports now prefer Android provider `DISPLAY_NAME` for each queued `content://` or `file://` item, so multi-file shares from Telegram, LINE, and Discord keep meaningful filenames.
- Shared local-file imports now pass the `content://` or `file://` URI through the service intent data/ClipData with read permission, making background import from Telegram, LINE, and Discord providers more reliable.
- Interrupted `content://` shared-file imports are marked as needing the file to be shared again after service restart instead of being automatically requeued after the provider grant may have expired.
- Manual retry now skips failed `content://` shared-file imports that require the source app to share the file again, preventing expired provider grants from cycling back into the queue.
- Android input accepts Discord attachment links through `ACTION_VIEW` for Discord CDN/media hosts such as `cdn.discordapp.com` and `media.discordapp.net`, pre-filling the URL and inferred filename for the same download flow.
- Android now registers generic `ACTION_VIEW` handlers for `content://` and `file://` files, so file managers and chat apps can open a local/shared file directly in the downloader with the provider display name preserved.
- Incoming `content://` shares and file-open intents now persist read permission when the provider grants persistable access, improving background import survival for document/file-manager sources while keeping transient chat-app grants on the existing ClipData path.
...
- Android search now continues past a small direct-site result set to fill candidates from search-engine fallback, scores CJK title overlap more flexibly, and accepts broader lazy-image/preview/JSON thumbnail fields with escaped URL normalization.
